...
actual class BlazeDB private constructor(
    ...
    actual companion object {
        actual fun open(path: String, password: String): BlazeDB {
            val nativeHandle = BlazeDBBridge.nativeOpen(path, password)
            require(nativeHandle > 0) { "BlazeDB.open failed ($nativeHandle)" }
            return BlazeDB(nativeHandle, path, password)
        }
    }
}
