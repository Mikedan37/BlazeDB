...
class BlazeDBRuntimeSmokeTest {
    ...
    @Test
    fun observeOpenTodosReusesOpenHandle() = runBlocking {
        val context = InstrumentationRegistry.getInstrumentation().targetContext
        ...
        val db = BlazeDB.open(dbFile.absolutePath, MainActivity.DEMO_PASSWORD)
        try {
            assertEquals(0, db.putTodo(Todo(title = "live-kmm")))
            val todos = withTimeout(3_000) {
                db.observeOpenTodos().first()
            }
            assertTrue(todos.any { it.title == "live-kmm" })
        } finally {
            db.close()
        }
    }
