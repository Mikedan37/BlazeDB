...
public final class BlazeDBClient: @unchecked Sendable {
    ...
    /// Clear all cached password-derived encryption keys (useful for testing).
    ...
    ///
    /// The `path` parameter is retained for source compatibility. BlazeDB no longer caches
    /// encryption keys by path because path-only reuse can bypass password validation.
    ...
    public init(name: String, fileURL: URL, password: String, project: String = "Default") throws {
        ...
        // 🔑 Derive the key from the supplied password and this database's salt.
