//
//  BlazeDataQuery.swift
//  BlazeDB
//
//  SwiftUI property wrapper for reactive **untyped** ``BlazeDataRecord`` lists (facade only).
//  Prefer ``BlazeQuery`` when your model conforms to ``BlazeDocument``.
//
...
// MARK: - BlazeDataQuery Property Wrapper

/// A SwiftUI property wrapper that automatically executes and updates database queries, returning raw ``BlazeDataRecord`` rows.
///
/// For typed SwiftUI models, use ``BlazeQuery`` (``BlazeDocument``) or ``BlazeStorableQuery`` (``BlazeStorable`` / Codable)—same change-observation idea, different model protocols. All use the same underlying ``BlazeDBClient/query()`` engine.
///
/// Example:
/// ```swift
...
@propertyWrapper
@MainActor
public struct BlazeDataQuery: DynamicProperty {
    ...
    public func typed<T: BlazeDocument>(as type: T.Type) throws -> [T] {
        try observer.results.map { try T(from: $0) }
    }
...
// MARK: - BlazeQuery Observer (raw records)

/// Observable object that manages query execution and result updates for ``BlazeDataQuery``.
@MainActor
    ...
    public func refresh() {
        ...
        refreshTask = Task { @MainActor in
            ...
            do {
                ...
                BlazeLogger.debug("@BlazeDataQuery fetched \(records.count) records")

                self.results = records
                self.isLoading = false

            } catch {
                BlazeLogger.error("@BlazeDataQuery fetch failed: \(error)")
                self.error = error
                self.isLoading = false
            }
        }
...
extension View {
    /// Trigger a refresh of a ``BlazeDataQuery`` when this view appears
    ...
    /// Enable pull-to-refresh for a ``BlazeDataQuery``
