...
internal protocol RLSPolicy: Sendable {
...
internal struct BlazeRLSRegistry {
    ...
    /// ```
    internal static func registerRLS<T, P: RLSPolicy>(
        _ type: T.Type,
        policy: P
    ) where P.Model == T {
        lock.lock()
    ...
    internal static func unregisterRLS<T>(_ type: T.Type) {
        lock.lock()
        defer { lock.unlock() }
...
internal struct TeamBasedRLSPolicy<T>: RLSPolicy {
    internal typealias Model = T
    
    private let teamIDField: String
    
    internal init(teamIDField: String = "team_id") {
        self.teamIDField = teamIDField
    }
    
    internal func filter(for user: BlazeUserContext) -> (BlazeDataRecord) -> Bool {
...
internal struct OwnerBasedRLSPolicy<T>: RLSPolicy {
    internal typealias Model = T
    
    private let ownerIDField: String
    
    internal init(ownerIDField: String = "owner_id") {
...
internal struct RoleBasedRLSPolicy<T>: RLSPolicy {
    internal typealias Model = T
    
    private let teamIDField: String
    private let assignedToField: String
    
    internal init(teamIDField: String = "team_id", assignedToField: String = "assigned_to") {
        self.teamIDField = teamIDField
