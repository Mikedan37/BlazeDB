...
final class SchemaMigrationTests: XCTestCase {
    ...
    func testMigrationPlan_InvalidOrder_AutosortsAndSucceeds() {
        let current = SchemaVersion(major: 1, minor: 0)
        let target = SchemaVersion(major: 1, minor: 2)
        let migrations: [BlazeDBMigration] = [
        ...
        XCTAssertTrue(plan.isValid, "Planner sorts migrations by source version")
        XCTAssertEqual(plan.migrations.count, 2, "Sorted migration chain should be complete")
    }
