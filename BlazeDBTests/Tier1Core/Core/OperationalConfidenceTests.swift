...
final class OperationalConfidenceTests: XCTestCase {
    ...
    func testHealth_WARN_LargeWAL() {
        ...
        XCTAssertTrue(health.reasons.contains { $0.localizedCaseInsensitiveContains("wal") })
    }
    ...
    func testHealth_WARN_VeryLowCacheHitRate() {
        let stats = DatabaseStats(
            pageCount: 100,
            walSize: nil,
            lastCheckpoint: nil,
        ...
        let health = HealthAnalyzer.analyze(stats)
        XCTAssertEqual(health.status, .warn, "Very low cache hit rate should currently classify as WARN")
        XCTAssertTrue(health.reasons.contains { $0.contains("Cache hit rate") })
    }
