...
final class SecureConnectionTests: XCTestCase {
    ...
    func testKeyDerivation() throws {
        ...
        let clientIKM = SymmetricKey(data: clientSecret.withUnsafeBytes { Data($0) })
        let serverIKM = SymmetricKey(data: serverSecret.withUnsafeBytes { Data($0) })
        
        let clientKey = HKDF<SHA256>.deriveKey(
            inputKeyMaterial: clientIKM,
            salt: salt,
            info: info,
            outputByteCount: 32
        )
        ...
        // Keys should match
        let clientBytes = clientKey.withUnsafeBytes { Data($0) }
        let serverBytes = serverKey.withUnsafeBytes { Data($0) }
        XCTAssertEqual(clientBytes, serverBytes)
    }
