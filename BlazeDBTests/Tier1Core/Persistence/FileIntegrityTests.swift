...
final class FileIntegrityTests: XCTestCase {
    ...
    func testMetadataAlwaysValidJSON() throws {
        ...
        let stages: [(String, () throws -> Void)] = [
            ...
            ("After reopen", {
                self.db = nil
                BlazeDBClient.clearCachedKey()
                self.db = try BlazeDBClient(name: "file_test", fileURL: self.tempURL, password: "SecureTestDB-456!")
            })
        ]
    ...
    func testNoFileDescriptorLeaks() throws {
        ...
        XCTAssertEqual(try db.count(), 20, "All 20 records from 20 cycles should exist")
    }
}
