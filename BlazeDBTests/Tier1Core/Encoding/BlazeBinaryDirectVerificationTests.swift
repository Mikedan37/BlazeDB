...
final class BlazeBinaryDirectVerificationTests: XCTestCase {
        ...
        for (index, value) in criticalValues.enumerated() {
            ...
            case .vector(let expected):
                if case .vector(let decodedV) = decodedValue {
                    XCTAssertEqual(decodedV.count, expected.count, "Vector \(index) corrupted!")
                } else {
                    XCTFail("Vector \(index) corrupted!")
                }
            case .null:
                XCTAssertNotNil(decodedValue, "Null \(index) corrupted!")
            }
        }
    ...
    func testDirect_DetectsByteCorruption_Exactly() throws {
        ...
        for corruptIndex in [0, 5, 10, 20, 30] {
            ...
            do {
                ...
                // At least one field should differ or be missing - explicit capture for Swift 6
                let decodedStorage = decoded.storage
                let originalStorage = originalDecoded.storage
                let matches = decodedStorage.keys.allSatisfy { key in
                    decodedStorage[key] == originalStorage[key]
                }
    ...
    func testDirect_FormatStability_AcrossRuns() throws {
        ...
        // Encode 10 times - explicit capture for Swift 6
        let encodings = try (0..<10).map { [record] _ in
