...
final class BatchOperationTests: XCTestCase {
    ...
    override func setUp() {
        ...
        db = try! BlazeDBClient(name: "batch_test_\(testID)", fileURL: tempURL, password: "Batch-Test-1234!")
        
        // ✅ SAFETY: Rollback any leftover transaction from previous test
        try? db.rollbackTransaction()
        
        // Verify database starts completely empty
        let startCount = (try? db.count()) ?? 0
        if startCount != 0 {
    ...
    func testInsertManyPerformance() throws {
        ...
        try db.persist()  // Flush metadata before count
        XCTAssertEqual(try db.count(), 1000)
    }
    ...
    func testDeleteMany() throws {
        ...
        try db.persist()  // Flush metadata before count
        XCTAssertEqual(try db.count(), 50)
    }
    ...
    func testDeleteManyByDate() throws {
        ...
        try db.persist()  // Flush metadata before count
        XCTAssertEqual(try db.count(), 30)
    }
    ...
    func testBatchInsertThenBatchDelete() throws {
        ...
        try db.persist()  // Flush metadata before count
        XCTAssertEqual(try db.count(), 25)
    }
    ...
    func testBatchInsertRollback() throws {
        ...
        XCTAssertEqual(try db.count(), 1)  // Only original record remains
    }
    ...
    func testConcurrentBatchInserts() throws {
        ...
        for batchNum in 0..<5 {
            ...
            let currentCount = try db.count()
            print("  Batch \(batchNum): inserted \(ids.count) records, returned \(ids.count) IDs, try db.count() = \(currentCount), expected = \(expectedTotal)")
    ...
    func testLargeBatchInsert() throws {
        ...
        try db.persist()  // Flush metadata before count
        XCTAssertEqual(try db.count(), 5000)
