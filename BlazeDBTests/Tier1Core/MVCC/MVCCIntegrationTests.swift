...
final class MVCCIntegrationTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        db = try BlazeDBClient(name: "mvcc_integ_test", fileURL: tempURL, password: "SecureTestDB-456!")
    }
    ...
    func testMVCC_SnapshotConsistency() throws {
        ...
        DispatchQueue.global().async {
            ...
            count1 = (try? self.db.count()) ?? 0
            Thread.sleep(forTimeInterval: 0.1)
            // Count should be same (snapshot isolation)
            count2 = (try? self.db.count()) ?? 0
        }
        ...
        print("  📊 Final count: \(try db.count())")
