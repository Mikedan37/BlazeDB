...
final class TelemetryUnitTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        db = try BlazeDBClient(name: "TelemetryTest", fileURL: dbURL, password: "SecureTestDB-456!")
    }
...
final class TelemetryUnitTests: XCTestCase {
    func testTelemetryStubUnavailableInCoreBuild() throws {
        ...
        let db = try BlazeDBClient(name: "TelemetryStub", fileURL: dbURL, password: "SecureTestDB-456!")
        XCTAssertFalse(db.telemetry.isAvailable, "Core builds should expose no-op telemetry stub")
        db.telemetry.enable(samplingRate: 1.0) // no-op, should not throw/crash
    }
}
#endif
