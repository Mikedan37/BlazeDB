...
final class VacuumOperationsTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        db = try BlazeDBClient(name: "VacuumTest", fileURL: dbURL, password: "SecureTestDB-456!")
    }
    ...
    func testVacuumEncodingRoundTrip() async throws {
        ...
        let alice = try await db.fetch(id: id1)
        ...
        let bob = try await db.fetch(id: id2)
        ...
        _ = try await db.vacuum()
        let afterSecond = try await db.fetchAll()
        XCTAssertEqual(afterSecond.count, 2, "Second vacuum should not lose records")

        let aliceAgain = try await db.fetch(id: id1)
