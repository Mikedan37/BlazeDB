...
final class LifecycleTests: XCTestCase {
    ...
    func testClose_IsIdempotent() throws {
        let db = try BlazeDBClient(name: "test", fileURL: dbURL, password: "TestPassword-123!")
    ...
    func testClose_FlushesPendingChanges() throws {
        ...
        let reopenedDB = try BlazeDBClient(name: "test", fileURL: dbURL, password: "TestPassword-123!")
    ...
    func testDeinit_AutoCloses() throws {
        var db: BlazeDBClient? = try BlazeDBClient(name: "test", fileURL: dbURL, password: "TestPassword-123!")
    ...
    func testSecondOpenSamePath_ThrowsConcurrentProcessAccessNotSupported() throws {
        ...
        do {
            _ = try BlazeDBClient(name: "other", fileURL: dbURL, password: "TestPassword-123!")
            XCTFail("Expected BlazeDBError.concurrentProcessAccessNotSupported when opening same path while first handle is open")
            ...
            XCTFail("Expected BlazeDBError.concurrentProcessAccessNotSupported, got: \(error)")
        }
    }
}
