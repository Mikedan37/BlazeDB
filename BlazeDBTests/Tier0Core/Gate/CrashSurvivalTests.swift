...
final class CrashSurvivalTests: XCTestCase {
    ...
    func testPowerLoss_AllCommittedDataSurvives() throws {
        ...
        // Write data
        var db: BlazeDBClient? = try BlazeDBClient(name: "power-loss", fileURL: dbURL, password: "TestPassword-123!")
        ...
        for i in 0..<recordCount {
            ...
            if let id = try db?.insert(record) {
                insertedIDs.append(id)
            } else {
                XCTFail("Failed to insert record during crash-survival setup")
            }
        ...
        // Reopen database
        let reopenedDB = try BlazeDBClient(name: "power-loss", fileURL: dbURL, password: "TestPassword-123!")
    ...
    func testCrashDuringTransaction_UncommittedDataRolledBack() throws {
        ...
        var db: BlazeDBClient? = try BlazeDBClient(name: "txn-crash", fileURL: dbURL, password: "TestPassword-123!")
        ...
        // Reopen database
        let reopenedDB = try BlazeDBClient(name: "txn-crash", fileURL: dbURL, password: "TestPassword-123!")
    ...
    func testCrashRecovery_HealthStatusValid() throws {
        ...
        var db: BlazeDBClient? = try BlazeDBClient(name: "health-test", fileURL: dbURL, password: "TestPassword-123!")
        for i in 0..<50 {
        ...
        // Recover
        let recoveredDB = try BlazeDBClient(name: "health-test", fileURL: dbURL, password: "TestPassword-123!")
    ...
    func testWALReplay_NoDuplicateRecords() throws {
        ...
        // Write records
        var db: BlazeDBClient? = try BlazeDBClient(name: "wal-replay", fileURL: dbURL, password: "TestPassword-123!")
        ...
        let recoveredDB = try BlazeDBClient(name: "wal-replay", fileURL: dbURL, password: "TestPassword-123!")
