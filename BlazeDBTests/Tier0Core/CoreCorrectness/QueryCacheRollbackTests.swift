...
final class QueryCacheRollbackTests: XCTestCase {
    ...
    func testQueryAsyncCacheDoesNotReturnRolledBackInsert() async throws {
        ...
        // In an async test method, bare `insert` resolves to the async overload —
        // always await it (this is what broke the Cursor bot PRs on macOS).
        _ = try await db.insert(BlazeDataRecord(["status": .string("open"), "marker": .string("baseline")]))

        try db.beginTransaction()
        _ = try await db.insert(BlazeDataRecord(["status": .string("open"), "marker": .string("rolled-back")]))
