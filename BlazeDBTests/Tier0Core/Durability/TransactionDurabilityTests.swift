...
final class TransactionDurabilityTests: XCTestCase {
    ...
    private func tmpDir(_ name: String = UUID().uuidString) -> URL {
        ...
        // Include a unique suffix to avoid cross-run and cross-target collisions.
        let dir = base.appendingPathComponent("blazedb-tests-\(name)-\(UUID().uuidString)", isDirectory: true)
    ...
    private func testKey(_ scope: String) -> SymmetricKey {
        // Stable key per test scope so reopen/recovery assertions compare actual storage behavior.
        var seed = Array(scope.utf8)
        if seed.count < 32 {
            seed.append(contentsOf: Array(repeating: UInt8(0), count: 32 - seed.count))
        }
        return SymmetricKey(data: Data(seed.prefix(32)))
    }

    private func makeStore(at url: URL, key: SymmetricKey) throws -> PageStore {
        return try PageStore(fileURL: url, key: key)
    }
    ...
    func testWALLogContainsEntriesPreCommitAndClearsAfterCommit() throws {
        ...
        let key = testKey("wal-presence")
        let store: PageStore = try makeStore(at: url, key: key)
    ...
    func testCrashRecovery_NoPartialOutcomes_AllOrNothing() throws {
        ...
        let url = dbURL(in: dir)
        let key = testKey("wal-crash-invariant")
        ...
        let store2: PageStore = try makeStore(at: url, key: key)
    ...
    func testConcurrentWritesToSamePageSerializeFinalStateIsConsistent() throws {
        ...
        let url = dbURL(in: dir)
        let key = testKey("wal-concurrency-write")
        let store: PageStore = try makeStore(at: url, key: key)
    ...
    func testConcurrentReadsAllowedWhileWriterOperates() throws {
        ...
        let url = dbURL(in: dir)
        let key = testKey("wal-concurrency-read")
        let store: PageStore = try makeStore(at: url, key: key)
    ...
    func testWALFileGrowthAndCleanup() throws {
        ...
        let db = try BlazeDBClient(name: "WALGrowthTest", fileURL: tempURL, password: "TestPassword-123!")
