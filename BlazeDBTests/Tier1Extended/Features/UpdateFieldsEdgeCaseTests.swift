...
final class UpdateFieldsEdgeCaseTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        db = try BlazeDBClient(name: "UpdateFieldsTest_\(testID)", fileURL: try requireFixture(tempURL), password: "TestPassword-123!")
    }
    ...
    func testUpdateFieldsWithSearchIndex() throws {
        ...
        let results1 = try requireFixture(db).query().search("original", in: ["title", "body"])
        ...
        let results2 = try requireFixture(db).query().search("original title", in: ["title", "body"])
        ...
        let results3 = try requireFixture(db).query().search("updated", in: ["title", "body"])
    ...
    func testUpdateFieldsInTransactionWithRollback() throws {
        ...
        let inTx = try requireFixture(db).fetch(id: id)
        ...
        let afterRollback = try requireFixture(db).fetch(id: id)
