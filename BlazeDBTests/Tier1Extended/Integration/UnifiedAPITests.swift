...
final class UnifiedAPITests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        db = try BlazeDBClient(name: "unified_test_\(testID)", fileURL: try requireFixture(tempURL), password: "SecureTestDB-456!")
    }
    ...
    func testJoinQuery_CanFlattenToRecords() throws {
        ...
        let usersDB = try BlazeDBClient(name: "users2", fileURL: try requireFixture(usersURL), password: "SecureTestDB-456!")
    ...
    func testAggregationQuery_ReturnsAggregationResult() throws {
        ...
        _ = try requireFixture(db).insert(BlazeDataRecord(["value": .int(30)]))
    ...
    func testGroupedAggregation_ReturnsGroupedResult() throws {
        // Insert test data
        _ = try requireFixture(db).insert(BlazeDataRecord(["category": .string("A"), "value": .int(10)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["category": .string("A"), "value": .int(20)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["category": .string("B"), "value": .int(30)]))
    ...
    func testIsTypeMethod() throws {
        // Standard query
        let standard = try requireFixture(db).query().execute()
