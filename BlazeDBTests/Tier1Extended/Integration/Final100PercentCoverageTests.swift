...
final class Final100PercentCoverageTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        db = try BlazeDBClient(name: "Final100Test_\(testID)", fileURL: url, password: "TestPassword-123!")
    }
    ...
    func testCheckIntegrityHealthyDatabase() throws {
    ...
    func testValidateIntegrityStrictMode() throws {
        ...
        let report1 = try requireFixture(db).validateDatabaseIntegrity(strict: false)
        XCTAssertNotNil(report1, "Should return validation report")
        
        // Strict mode
        let report2 = try requireFixture(db).validateDatabaseIntegrity(strict: true)
    ...
    func testPerformSafeWriteReentrancyProtection() throws {
        ...
        let id = try requireFixture(db).insert(BlazeDataRecord(["value": .int(1)]))
        
        // Update should work (also uses performSafeWrite)
        try requireFixture(db).update(id: id, with: BlazeDataRecord(["value": .int(2)]))
    ...
    func testReplayCorruptedTransactionLog() throws {
        ...
        // Insert initial data
        let id1 = try requireFixture(db).insert(BlazeDataRecord(["value": .int(1)]))
        ...
        let logURL = try requireFixture(tempURL).deletingLastPathComponent()
        ...
        let fetched = try requireFixture(db).fetch(id: id1)
    ...
    func testDestroyRemovesAllFiles() throws {
        ...
        // Verify files exist
        XCTAssertTrue(FileManager.default.fileExists(atPath: try requireFixture(tempURL).path), 
                     "Data file should exist")
        XCTAssertTrue(FileManager.default.fileExists(atPath: try requireFixture(tempURL).deletingPathExtension()
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: try requireFixture(tempURL).path), 
                      "Data file should be removed")
        XCTAssertFalse(FileManager.default.fileExists(atPath: try requireFixture(tempURL).deletingPathExtension()
                                                             .appendingPathExtension("meta").path), 
    ...
    func testOperationsAfterDestroyFail() throws {
        ...
        _ = try requireFixture(db).insert(BlazeDataRecord(["after": .string("destroy")]))
    ...
    func testPurgeRemovesMultipleSoftDeleted() throws {
        ...
        for id in ids.prefix(5) {
            try requireFixture(db).softDelete(id: id)
        }
        
        print("  Soft deleted 5 records")
        
        // Soft-deleted records are hidden from fetchAll — only 5 non-deleted visible
        XCTAssertEqual(try requireFixture(db).fetchAll().count, 5, "Should see only 5 non-deleted before purge")

        // Purge permanently removes soft-deleted records from disk
        try requireFixture(db).purge()

        print("  Purged soft-deleted records")

        // After purge, still 5 visible (disk space reclaimed)
        XCTAssertEqual(try requireFixture(db).fetchAll().count, 5, "Should have 5 after purge")
        
        // Verify the right ones were deleted
        for id in ids.suffix(5) {
            XCTAssertNotNil(try requireFixture(db).fetch(id: id), "Non-deleted records should remain")
        }
    ...
    func testFetchWithAllAnyHashableTypes() throws {
        ...
        // Insert with String
        _ = try requireFixture(db).insert(BlazeDataRecord(["value": .string("test"), "type": .string("string")]))
        
        // Insert with Int
        _ = try requireFixture(db).insert(BlazeDataRecord(["value": .int(42), "type": .string("int")]))
        
        // Insert with Double
        _ = try requireFixture(db).insert(BlazeDataRecord(["value": .double(3.14), "type": .string("double")]))
        
        // Insert with Bool
        _ = try requireFixture(db).insert(BlazeDataRecord(["value": .bool(true), "type": .string("bool")]))
        
        // Insert with Date
        let testDate = Date()
        _ = try requireFixture(db).insert(BlazeDataRecord(["value": .date(testDate), "type": .string("date")]))
        
        // Insert with UUID
        let testUUID = UUID()
        _ = try requireFixture(db).insert(BlazeDataRecord(["value": .uuid(testUUID), "type": .string("uuid")]))
        
        // Insert with Data
        let testData = Data([0x01, 0x02])
        _ = try requireFixture(db).insert(BlazeDataRecord(["value": .data(testData), "type": .string("data")]))
    ...
    func testCrashSimulationEnvVar() {
        ...
        if let id = id {
            do {
                try requireFixture(db).update(id: id, with: BlazeDataRecord(["test": .string("updated")]))
    ...
    func testUpdateManyNoMatches() throws {
        ...
        // Insert data
        for i in 0..<5 {
            _ = try requireFixture(db).insert(BlazeDataRecord(["status": .string("active"), "index": .int(i)]))
        }
    ...
    func testDeleteManyNoMatches() throws {
        ...
        for i in 0..<5 {
            _ = try requireFixture(db).insert(BlazeDataRecord(["type": .string("keep"), "index": .int(i)]))
        }
        
        let count = try requireFixture(db).deleteMany(where: { $0.storage["type"]?.stringValue == "remove" })
        
        XCTAssertEqual(count, 0, "Should delete 0 records when none match")
        XCTAssertEqual(try requireFixture(db).fetchAll().count, 5, "All records should remain")
    ...
    func testFetchOnNonExistentIndex() throws {
        ...
        _ = try requireFixture(db).insert(BlazeDataRecord(["field": .string("value")]))
