...
final class BlazeDBInitializationTests: XCTestCase {
    ...
    func testFailableInit_Success() throws {
        ...
        var db: BlazeDBClient? = BlazeDBClient(name: "TestDB", at: try XCTUnwrap(tempURL), password: "SecurePassword-123!")
    ...
    func testFailableInit_WeakPassword() throws {
        ...
        let db = BlazeDBClient(name: "TestDB", at: try XCTUnwrap(tempURL), password: "weak")
    ...
    func testFailableInit_InvalidPath() throws {
        ...
        // Should return nil (not crash)
        let db = BlazeDBClient(name: "TestDB", at: invalidURL, password: "SecurePassword-123!")
    ...
    func testFailableInit_VariousWeakPasswords() throws {
        ...
        for password in weakPasswords {
            let db = BlazeDBClient(name: "TestDB", at: try XCTUnwrap(tempURL), password: password)
    ...
    func testFailableInit_MinimumValidPassword() throws {
        print("🔷 Testing failable initializer with minimum valid password...")
        
        // Password must be 12+ chars with uppercase, lowercase, and numbers
        var db: BlazeDBClient? = BlazeDBClient(name: "TestDB", at: try XCTUnwrap(tempURL), password: "MinPass-12AB!")
        
        guard let dbUnwrapped = db else {
            XCTFail("Should accept minimum valid password (12+ chars, mixed case, numbers)")
            return
        }
    ...
    func testErrorMessages_AreHelpful() throws {
        ...
        do {
            _ = try BlazeDBClient(name: "TestDB", fileURL: try requireFixture(tempURL), password: "bad")
    ...
    func testConvenienceInit_Throwing() throws {
        ...
        var db: BlazeDBClient? = try BlazeDBClient(name: "TestDB", fileURL: try requireFixture(tempURL), password: "SecurePassword-123!")
    ...
    func testRealWorld_ErrorRecovery() throws {
        ...
        db = BlazeDBClient(name: "TestDB", at: try XCTUnwrap(tempURL), password: "weak")
        XCTAssertNil(db, "First attempt should fail")
        
        // Retry with strong password (simulating retry logic)
        db = BlazeDBClient(name: "TestDB", at: try XCTUnwrap(tempURL), password: "StrongPassword-123!")
        ...
        // Clean up explicitly - must close before tearDown
        try? db?.persist()
        try? db?.close()
        db = nil
