...
final class FileIntegrityTests: XCTestCase {
    ...
    private func fileIntegrityURL() throws -> URL {
        try XCTUnwrap(tempURL, "tempURL should be set in setUpWithError")
    }
    
    private func fileIntegrityDB() throws -> BlazeDBClient {
    ...
    override func setUpWithError() throws {
        ...
        db = try BlazeDBClient(name: "file_test", fileURL: url, password: "SecureTestDB-456!")
    }
    ...
    func testMetadataChecksumStable() throws {
        ...
        let metaURL = try fileIntegrityURL().deletingPathExtension().appendingPathExtension("meta")
    ...
    func testDataFileChecksumStable() throws {
        ...
        let hash1 = try sha256(of: try fileIntegrityURL())
        
        // Persist again (no changes)
        try fileIntegrityDB().persist()
        let hash2 = try sha256(of: try fileIntegrityURL())
    ...
    func testMetadataFileDoesNotGrowUnexpectedly() throws {
        ...
        try fileIntegrityDB().persist()
        let size1 = try fileAttributeIntSize(atPath: metaURL.path)
        
        // Reopen
        db = nil
        Thread.sleep(forTimeInterval: 0.1)
        BlazeDBClient.clearCachedKey()
        db = try BlazeDBClient(name: "file_test", fileURL: try fileIntegrityURL(), password: "SecureTestDB-456!")
        
        let size2 = try fileAttributeIntSize(atPath: metaURL.path)
        ...
        // Persist again
        try fileIntegrityDB().persist()
        let size3 = try fileAttributeIntSize(atPath: metaURL.path)
    ...
    func testDataFileSizeConsistent() throws {
        ...
        let fileSize = try fileAttributeIntSize(atPath: try fileIntegrityURL().path)
    ...
    func testMetadataAlwaysValidJSON() throws {
        ...
        let stages: [(String, () throws -> Void)] = [
            ...
            ("After reopen", {
                self.db = nil
                BlazeDBClient.clearCachedKey()
                self.db = try BlazeDBClient(name: "file_test", fileURL: try self.fileIntegrityURL(), password: "SecureTestDB-456!")
            })
        ]
    ...
    func testNoFileDescriptorLeaks() throws {
        ...
        XCTAssertEqual(try fileIntegrityDB().count(), 20, "All 20 records from 20 cycles should exist")
    }
}
