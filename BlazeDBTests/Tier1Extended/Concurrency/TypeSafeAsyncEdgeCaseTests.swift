...
final class TypeSafeAsyncEdgeCaseTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        // Create unique database file with timestamp + UUID (avoid Thread.current.hash in async)
        let testID = "\(UUID().uuidString)-\(UUID().hashValue)-\(Date().timeIntervalSince1970)"
        ...
        db = try BlazeDBClient(name: "type_async_test_\(testID)", fileURL: try requireFixture(tempURL), password: "TestPassword-123!")
    ...
    func testAsyncInsertTypeSafe() async throws {
        ...
        let id = try await requireFixture(db).insert(model)
        
        XCTAssertEqual(id, model.id)
        
        let fetched = try await requireFixture(db).fetch(AsyncTestModel.self, id: id)
    ...
    func testAsyncFetchNonExistentTypeSafe() async throws {
        ...
        let randomID = UUID()
        let fetched = try await requireFixture(db).fetch(AsyncTestModel.self, id: randomID)
    ...
    func testAsyncFetchAllTypeSafe() async throws {
        ...
        for i in 0..<5 {
            ...
            _ = try await requireFixture(db).insert(model)
        }
        
        // Fetch all as type
        let all = try await requireFixture(db).fetchAll(AsyncTestModel.self)
    ...
    func testAsyncUpdateTypeSafe() async throws {
        ...
        model.name = "Updated"
        model.count = 2
        try await requireFixture(db).update(model)
        
        // Verify
        let fetched = try await requireFixture(db).fetch(AsyncTestModel.self, id: model.id)
    ...
    func testAsyncUpsertTypeSafe() async throws {
        ...
        // First upsert (insert)
        let wasInsert = try await requireFixture(db).upsert(model)
        XCTAssertTrue(wasInsert, "First upsert should be insert")
        
        // Second upsert (update)
        let wasInsert2 = try await requireFixture(db).upsert(model)
    ...
    func testAsyncInsertManyTypeSafe() async throws {
        ...
        let ids = try await requireFixture(db).insertMany(models)
