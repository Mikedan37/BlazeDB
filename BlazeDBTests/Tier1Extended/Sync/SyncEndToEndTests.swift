...
final class SyncEndToEndTests: XCTestCase {
    ...
    func testLocalSyncEndToEnd() async throws {
        ...
        let db1 = try BlazeDBClient(name: "DB1", fileURL: db1URL, password: "TestPass123!")
        let db2 = try BlazeDBClient(name: "DB2", fileURL: db2URL, password: "TestPass123!")
        ...
        let recordId = try requireFixture(db1).insert(record)
    ...
    func testUpdateSyncEndToEnd() async throws {
        ...
        // Update in db1
        var updated = try requireFixture(db1).fetch(id: recordId)!
        updated.storage["title"] = .string("Updated")
        updated.storage["value"] = .int(2)
        try requireFixture(db1).update(id: recordId, with: updated)
    ...
    func testDeleteSyncEndToEnd() async throws {
        ...
        XCTAssertNotNil(try requireFixture(db2).fetch(id: recordId))
        
        // Delete in db1
        try requireFixture(db1).delete(id: recordId)
        
        // Wait for delete sync
        try await Task.sleep(nanoseconds: 500_000_000)
        
        // Verify deleted in db2
        XCTAssertNil(try requireFixture(db2).fetch(id: recordId))
    }
    ...
    func testBidirectionalSyncEndToEnd() async throws {
        ...
        // Insert in db1
        let id1_record = try requireFixture(db1).insert(BlazeDataRecord([
            "source": .string("db1"),
            "value": .int(1)
        ]))
        
        // Insert in db2
        let id2_record = try requireFixture(db2).insert(BlazeDataRecord([
            "source": .string("db2"),
            "value": .int(2)
        ]))
        ...
        XCTAssertNotNil(try requireFixture(db2).fetch(id: id1_record), "db1's record should be in db2")
        XCTAssertNotNil(try requireFixture(db1).fetch(id: id2_record), "db2's record should be in db1")
    }
}
#else
final class SyncEndToEndTests: XCTestCase {
    func testSyncEndToEndUnavailableInCoreBuild() {
        XCTAssertTrue(true, "End-to-end sync tests require distributed build")
    }
}
#endif
