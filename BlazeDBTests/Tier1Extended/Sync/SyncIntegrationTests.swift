...
final class SyncIntegrationTests: XCTestCase {
    ...
    func testMemorySafety_RapidOperations() async throws {
        ...
        for i in 0..<1000 {
            ...
            let id = try requireFixture(db1).insert(record)
            recordIds.append(id)
        }
    ...
    func testMemorySafety_ConcurrentOperations() async throws {
        ...
        let db1Count = try requireFixture(db1).query().all().count
        let db2Count = try requireFixture(db2).query().all().count
    ...
    func testPerformance_LargeBatchSync() async throws {
        ...
        let insertIds = try requireFixture(db1).insertMany(records)
    ...
    func testEdgeCase_RapidConnectDisconnect() async throws {
        ...
        // Insert a record
        let recordId = try requireFixture(db1).insert(BlazeDataRecord(["test": .string("value")]))
...
#else
final class SyncIntegrationTests: XCTestCase {
    func testSyncIntegrationUnavailableInCoreBuild() {
        XCTAssertTrue(true, "Sync integration tests require distributed build")
    }
}
#endif
