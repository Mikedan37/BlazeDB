...
final class UnixDomainSocketTests: XCTestCase {
    ...
    func testCrossAppSync_UnixDomainSocket() async throws {
        ...
        let db1 = try BlazeDBClient(name: "App1DB", fileURL: db1URL, password: "TestPass123!")
        let db2 = try BlazeDBClient(name: "App2DB", fileURL: db2URL, password: "TestPass123!")
        ...
        // Insert in db1
        let recordId = try requireFixture(db1).insert(BlazeDataRecord([
        ...
        let synced = try requireFixture(db2).fetch(id: recordId)
    ...
    func testCrossAppSync_Bidirectional() async throws {
        ...
        let db1 = try BlazeDBClient(name: "App1", fileURL: db1URL, password: "TestPass123!")
        let db2 = try BlazeDBClient(name: "App2", fileURL: db2URL, password: "TestPass123!")
        ...
        // Insert in both apps
        let id1_record = try requireFixture(db1).insert(BlazeDataRecord(["source": .string("app1"), "value": .int(1)]))
        let id2_record = try requireFixture(db2).insert(BlazeDataRecord(["source": .string("app2"), "value": .int(2)]))
        
        // Wait for sync
        try await Task.sleep(nanoseconds: 1_000_000_000)
        
        // Verify bidirectional sync
        XCTAssertNotNil(try requireFixture(db2).fetch(id: id1_record), "App1's record should be in App2")
        XCTAssertNotNil(try requireFixture(db1).fetch(id: id2_record), "App2's record should be in App1")
    }
    ...
    func testCrossAppSync_Performance() async throws {
        ...
        var recordIds: [UUID] = []
        for i in 0..<1000 {
            let id = try requireFixture(db1).insert(BlazeDataRecord([
                "index": .int(i),
        ...
        var syncedCount = 0
        for id in recordIds {
            if try requireFixture(db2).fetch(id: id) != nil {
                syncedCount += 1
            }
        }
...
#else
final class UnixDomainSocketTests: XCTestCase {
    func testUnixDomainSocketUnavailableInCoreBuild() {
        XCTAssertTrue(true, "Unix domain socket sync tests require distributed build")
    }
}
#endif
