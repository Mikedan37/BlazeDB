...
final class DistributedGCTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        db1 = try BlazeDBClient(name: "DB1", fileURL: url1, password: "SecureTestDB-456!")
        db2 = try BlazeDBClient(name: "DB2", fileURL: url2, password: "SecureTestDB-456!")
    }
    ...
    func testSyncStateGC_CleanupDeletedRecords() async throws {
        ...
        // Insert some records
        let id1 = try requireFixture(db1).insert(BlazeDataRecord(["value": .int(1)]))
        let id2 = try requireFixture(db1).insert(BlazeDataRecord(["value": .int(2)]))
        let id3 = try requireFixture(db1).insert(BlazeDataRecord(["value": .int(3)]))
        
        // Simulate sync state (manually set for testing)
        // In real usage, this would be set during sync
        
        // Delete one record
        try requireFixture(db1).delete(id: id2)
    ...
    func testIntegration_OperationLogGCWithSync() async throws {
        ...
        // Insert records
        let id = try requireFixture(db1).insert(BlazeDataRecord(["value": .int(1)]))
        
        // Wait for sync
        try await Task.sleep(nanoseconds: 100_000_000)  // 0.1 seconds
        
        // Verify sync
        let synced = try requireFixture(db2).fetch(id: id)
    ...
    func testIntegration_SyncStateGCWithMultipleRecords() async throws {
        ...
        var ids: [UUID] = []
        for i in 0..<10 {
            let id = try requireFixture(db1).insert(BlazeDataRecord(["value": .int(i)]))
            ids.append(id)
        }
        ...
        // Delete some records
        for i in 0..<5 {
            try requireFixture(db1).delete(id: ids[i])
        }
        ...
        for i in 5..<10 {
            let synced = try requireFixture(db2).fetch(id: ids[i])
...
#else
final class DistributedGCTests: XCTestCase {
    func testDistributedGCUnavailableInCoreBuild() {
        XCTAssertTrue(true, "Distributed GC tests require distributed build")
    }
}
#endif
