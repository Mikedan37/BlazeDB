...
final class DistributedSecurityTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        db1 = try BlazeDBClient(name: "db1", at: db1URL, password: "SecurePassword1-ABC!")
        db2 = try BlazeDBClient(name: "db2", at: db2URL, password: "SecurePassword2-XYZ!")
        
        validator = SecurityValidator(maxOperationsPerMinute: 1000)
    }
    
    override func tearDown() async throws {
        try? FileManager.default.removeItem(at: try requireFixture(tempDir))
    }
    
    // MARK: - Authentication Tests
    
    func testSyncEngine_InvalidRemoteOperation_DoesNotMutateState() async throws {
        let relayFrom = UUID()
        let relayTo = UUID()
        let relay = InMemoryRelay(
            fromNodeId: relayFrom,
            toNodeId: relayTo,
            mode: .bidirectional
        )
        
        // Default validator has no explicit permissions for remote node user ID.
        // This operation should fail validation and be rejected fail-closed.
        let engine = BlazeSyncEngine(
            localDB: db2,
            relay: relay,
            nodeId: relayTo,
            securityValidator: SecurityValidator(maxOperationsPerMinute: 1000)
        )
        ...
        let invalidRemoteOp = BlazeOperation(
            timestamp: LamportTimestamp(counter: 1, nodeId: relayFrom),
            nodeId: relayFrom,
            type: .insert,
            collectionName: try requireFixture(db2).name,
            recordId: targetRecordID,
            changes: ["title": .string("malicious insert")]
        )
        
        try await relay.pushOperations([invalidRemoteOp])
        
        // Allow relay callback task to run.
        try await Task.sleep(nanoseconds: 200_000_000)
        
        let fetched = try requireFixture(db2).fetch(id: targetRecordID)
        XCTAssertNil(fetched, "Invalid remote operation must not be applied to local state")
    }
    
    func testSyncEngine_MixedBatchWithInvalidOperation_RejectsEntireBatch() async throws {
        let relayFrom = UUID()
        let relayTo = UUID()
        let relay = InMemoryRelay(
            fromNodeId: relayFrom,
        ...
        let validLooking1 = BlazeOperation(
            timestamp: LamportTimestamp(counter: 1, nodeId: relayFrom),
            nodeId: relayFrom,
            type: .insert,
            collectionName: try requireFixture(db2).name,
            recordId: UUID(),
            changes: ["title": .string("op1")]
        )
        let invalid = BlazeOperation(
            timestamp: LamportTimestamp(counter: 2, nodeId: relayFrom),
            nodeId: relayFrom,
            type: .insert,
            collectionName: "forbidden_collection",
            recordId: UUID(),
            changes: ["title": .string("invalid")]
        )
        let validLooking2 = BlazeOperation(
            timestamp: LamportTimestamp(counter: 3, nodeId: relayFrom),
            nodeId: relayFrom,
            type: .insert,
            collectionName: try requireFixture(db2).name,
            recordId: UUID(),
            changes: ["title": .string("op3")]
        )
        
        try await relay.pushOperations([validLooking1, invalid, validLooking2])
        try await Task.sleep(nanoseconds: 250_000_000)
        
        XCTAssertNil(try requireFixture(db2).fetch(id: validLooking1.recordId), "Batch reject must prevent op1 mutation")
        XCTAssertNil(try requireFixture(db2).fetch(id: invalid.recordId), "Batch reject must prevent invalid op mutation")
        XCTAssertNil(try requireFixture(db2).fetch(id: validLooking2.recordId), "Batch reject must prevent op3 mutation")
    }
...
#else
final class DistributedSecurityTests: XCTestCase {
    func testDistributedSecurityUnavailableInCoreBuild() {
        XCTAssertTrue(true, "Distributed security tests require distributed build")
    }
}
#endif
