...
final class DataTypeCompoundIndexTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        db = try BlazeDBClient(name: "DataIdxTest_\(testID)", fileURL: try requireFixture(tempURL), password: "SecureTestDB-456!")
    }
    ...
    func testDataInCompoundIndex_Basic() throws {
        ...
        try requireFixture(db).collection.createIndex(on: ["name", "content"])
    ...
    func testDataAsCompoundIndexPrimaryKey() throws {
        ...
        try requireFixture(db).collection.createIndex(on: ["payload", "category"])
    ...
    func testDataWithUUIDInCompoundIndex() throws {
        ...
        try requireFixture(db).collection.createIndex(on: ["user_id", "signature"])
    ...
    func testMultipleDataFieldsInCompoundIndex() throws {
        ...
        try requireFixture(db).collection.createIndex(on: ["hash", "content"])
    ...
    func testDataFieldWithSearch() throws {
        ...
        // Search should work
        let searchResults = try requireFixture(db).query().search("document", in: ["title"])
    ...
    func testDataInCompoundIndexPersistence() throws {
        ...
        try requireFixture(db).collection.createIndex(on: ["category", "content"])
        try requireFixture(db).persist()
        
        // Close and reopen (use same file URL, not the name)
        let dbName = try requireFixture(db).name  // Capture the unique name
        db = nil
        
        // Reopen same database file with same name
        db = try BlazeDBClient(name: dbName, fileURL: try requireFixture(tempURL), password: "SecureTestDB-456!")
        
        print("  Reopened database, total records: \(try requireFixture(db).count())")
    ...
    func testEmptyDataInCompoundIndex() throws {
        ...
        try requireFixture(db).collection.createIndex(on: ["name", "data"])
    ...
    func testLargeDataInCompoundIndex() throws {
        ...
        try requireFixture(db).collection.createIndex(on: ["type", "payload"])
    ...
    func testDataFieldUpdateWithCompoundIndex() throws {
        ...
        try requireFixture(db).collection.createIndex(on: ["label", "content"])
    ...
    func testDataFieldDeleteWithCompoundIndex() throws {
        ...
        try requireFixture(db).collection.createIndex(on: ["tag", "blob"])
    ...
    func testDataInThreeFieldCompoundIndex() throws {
        ...
        try requireFixture(db).collection.createIndex(on: ["field1", "field2", "field3"])
