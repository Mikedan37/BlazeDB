...
final class FullTextSearchTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        db = try BlazeDBClient(name: "search_test_\(testID)", fileURL: try requireFixture(tempURL), password: "SecureTestDB-456!")
    }
    ...
    func testSimpleTextSearch() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Login bug"), "description": .string("Users can't login")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Signup issue"), "description": .string("Signup form broken")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Login button color"), "description": .string("Button is wrong color")]))
    ...
    func testSearchWithRelevanceScoring() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Login bug"), "body": .string("Some text")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Bug report"), "body": .string("Login issue here")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Login Login Login"), "body": .string("Triple match")]))
    ...
    func testCaseSensitiveSearch() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("LOGIN")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("login")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Login")]))
    ...
    func testMultiWordSearch() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Login bug on mobile")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Mobile bug report")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Login issue")]))
    ...
    func testPartialWordMatching() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Authentication failed")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Authorized users")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Author list")]))
    ...
    func testMultipleMatchesScoreHigher() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("login")]))  // 1 match
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("login login")]))  // 2 matches
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("login login login")]))  // 3 matches
    ...
    func testSearchNonStringField() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["value": .int(12345)]))
    ...
    func testSearchWithMinWordLength() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("A bug in the app")]))
    ...
    func testTokenizationWithPunctuation() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Bug: login, signup, logout!")]))
    ...
    func testTokenizationWithNumbers() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Bug #123: Login issue")]))
    ...
    func testTokenizationWithHyphens() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("User-authentication bug")]))
    ...
    func testSearchLargeText() throws {
        ...
        _ = try requireFixture(db).insert(BlazeDataRecord(["description": .string(largeText)]))
    ...
    func testSearchWithNilField() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["other": .string("value")]))  // No title field
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Login bug")]))
