...
// TestBug: shared from Tier1Core/Helpers/TypeSafetyTestBug.swift (via Helpers symlink).
...
final class TypeSafetyTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        db = try BlazeDBClient(name: "test_\(testID)", fileURL: try requireFixture(tempURL), password: "TestPassword-123!")
    ...
    func testFetchAllTypeSafeDocuments() async throws {
        ...
        try await requireFixture(db).insert(bug1)
        try await requireFixture(db).insert(bug2)
        try await requireFixture(db).insert(bug3)
    ...
    func testDeleteTypeSafeDocument() async throws {
        ...
        // Verify exists
        var fetched = try await requireFixture(db).fetch(TestBug.self, id: bug.id)
        XCTAssertNotNil(fetched)
        
        // Delete (same API as before)
        try await requireFixture(db).delete(id: bug.id)
        
        // Verify deleted
        fetched = try await requireFixture(db).fetch(TestBug.self, id: bug.id)
    ...
    func testInsertTypedFetchDynamic() async throws {
        ...
        let record = try await requireFixture(db).fetch(id: bug.id)
    ...
    func testUpdateDynamicFetchTyped() async throws {
        ...
        // Update dynamic
        var record = try await requireFixture(db).fetch(id: bug.id)!
        record["title"] = .string("Modified")
        record["priority"] = .int(10)
        try await requireFixture(db).update(id: bug.id, data: record)
    ...
    func testFetchWithMissingField_ThrowsError() async throws {
        ...
        do {
            let _ = try await requireFixture(db).fetch(TestBug.self, id: id)
    ...
    func testInvalidUUID_AutoHeals() async throws {
        ...
        let generatedID = try await requireFixture(db).insert(record)
        ...
        let fetched = try await requireFixture(db).fetch(TestBug.self, id: generatedID)
    ...
    func testInsertManyTypeSafe() async throws {
        ...
        // Fetch and verify
        let fetched = try await requireFixture(db).fetchAll(TestBug.self)
    ...
    func testUpsertTypeSafe_Insert() async throws {
        ...
        let wasInserted = try await requireFixture(db).upsert(bug)
    ...
    func testUpsertTypeSafe_Update() async throws {
        ...
        let wasInserted = try await requireFixture(db).upsert(updated)
    ...
    func testQueryResultTypeSafeConversion() async throws {
        // Insert typed bugs
        try await requireFixture(db).insert(TestBug(title: "Bug 1", priority: 1, status: "open"))
        try await requireFixture(db).insert(TestBug(title: "Bug 2", priority: 2, status: "closed"))
        try await requireFixture(db).insert(TestBug(title: "Bug 3", priority: 3, status: "open"))
    ...
    func testQueryWithTypeSafeSorting() async throws {
        ...
        try await requireFixture(db).insert(TestBug(title: "Bug C", priority: 3, status: "open"))
        try await requireFixture(db).insert(TestBug(title: "Bug A", priority: 1, status: "open"))
        try await requireFixture(db).insert(TestBug(title: "Bug B", priority: 2, status: "open"))
    ...
    func testComputedProperties() async throws {
        ...
        try await requireFixture(db).insert(highPriority)
        let fetched = try await requireFixture(db).fetch(TestBug.self, id: highPriority.id)
        ...
        measure {
            ...
            Task { [dbRef] in
                do {
                    ...
                    let _ = try await dbRef.db.insertMany(bugs)
                } catch {
    ...
    func testConcurrentTypeSafeFetches() async throws {
        ...
        await withTaskGroup(of: TestBug?.self) { group in
            for id in ids {
                group.addTask {
                    return try? await db.fetch(TestBug.self, id: id)
                }
            }
    ...
    func testSyncFetchTypeSafe() throws {
        let bug = TestBug(title: "Sync Bug", priority: 1, status: "open")
        try requireFixture(db).insert(bug)
        
        let fetched = try requireFixture(db).fetch(TestBug.self, id: bug.id)
        XCTAssertNotNil(fetched)
