...
final class DataTypeQueryTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        db = try BlazeDBClient(name: "DataQueryTest_\(testID)", fileURL: try requireFixture(tempURL), password: "TestPassword-123!")
    }
    ...
    func testOrderByDataDescending() throws {
        ...
        _ = try requireFixture(db).insert(BlazeDataRecord(["size": .string("A"), "data": .data(Data(count: 10))]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["size": .string("C"), "data": .data(Data(count: 100))]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["size": .string("B"), "data": .data(Data(count: 50))]))
    ...
    func testGroupByDataField() throws {
        ...
        _ = try requireFixture(db).insert(BlazeDataRecord(["group": .data(dataA), "value": .int(10)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["group": .data(dataA), "value": .int(20)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["group": .data(dataB), "value": .int(30)]))
    ...
    func testDataFieldsInJoinedRecords() throws {
        ...
        let db2 = try BlazeDBClient(name: "SecondDB", fileURL: url2, password: "TestPassword-123!")
        ...
        let allOrders = try requireFixture(db).fetchAll()
        print("  Orders in db1:")
        ...
        _ = try requireFixture(db2).insert(BlazeDataRecord([
            "id": .uuid(userId1),
            "name": .string("Alice"),
        ...
        let joined = try requireFixture(db).join(with: db2, on: "user_id", equals: "id", type: .inner)
    ...
    func testDistinctWithDataField() throws {
        ...
        // Insert duplicates
        _ = try requireFixture(db).insert(BlazeDataRecord(["binary": .data(data1), "index": .int(1)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["binary": .data(data1), "index": .int(2)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["binary": .data(data2), "index": .int(3)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["binary": .data(data2), "index": .int(4)]))
        
        // Get distinct binary values
        let distinct = try requireFixture(db).distinct(field: "binary")
