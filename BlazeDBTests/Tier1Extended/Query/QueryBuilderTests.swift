...
final class QueryBuilderTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        for _ in 0..<3 {
            ...
            if !FileManager.default.fileExists(atPath: try requireFixture(tempURL).path) { break }
            Thread.sleep(forTimeInterval: 0.01)
        }
        
        db = try BlazeDBClient(name: "query_test_\(testID)", fileURL: try requireFixture(tempURL), password: "SecureTestDB-456!")
    }
    ...
    func testWhereEquals() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["status": .string("open"), "title": .string("Bug 1")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["status": .string("closed"), "title": .string("Bug 2")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["status": .string("open"), "title": .string("Bug 3")]))
    ...
    func testWhereGreaterThan() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["priority": .int(1)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["priority": .int(5)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["priority": .int(10)]))
    ...
    func testWhereContains() throws {
        ...
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Slow query")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("Login timeout")]))
    ...
    func testWhereCustomClosure() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["priority": .int(1), "status": .string("open")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["priority": .int(5), "status": .string("open")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["priority": .int(10), "status": .string("closed")]))
    ...
    func testOrderByString() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["name": .string("Charlie")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["name": .string("Alice")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["name": .string("Bob")]))
    ...
    func testOrderByDate() throws {
        ...
        _ = try requireFixture(db).insert(BlazeDataRecord(["created_at": .date(date2)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["created_at": .date(date1)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["created_at": .date(date3)]))
    ...
    func testMultipleOrderBy() throws {
        ...
        _ = try requireFixture(db).insert(BlazeDataRecord(["priority": .int(1), "name": .string("Charlie")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["priority": .int(1), "name": .string("Alice")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["priority": .int(2), "name": .string("Bob")]))
    ...
    func testQueryBuilderFilterBeforeJoinOptimization() throws {
        ...
        _ = try requireFixture(usersDB).insert(BlazeDataRecord(["id": .uuid(userAlice), "name": .string("Alice")]))
        _ = try requireFixture(usersDB).insert(BlazeDataRecord(["id": .uuid(userBob), "name": .string("Bob")]))
    ...
    func testOrderByWithNilValues() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["priority": .int(5), "title": .string("Has priority")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["title": .string("No priority")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["priority": .int(1), "title": .string("Has priority 2")]))
    ...
    func testPaginationWithQueryBuilder() throws {
        ...
        // Page 1
        let page1 = try requireFixture(db).query()
            .where("status", equals: .string("open"))
            .orderBy("index", descending: false)
            .limit(10)
            .offset(0)
        ...
        // Page 2
        let page2 = try requireFixture(db).query()
            .where("status", equals: .string("open"))
            .orderBy("index", descending: false)
            .limit(10)
            .offset(10)
    ...
    func testCompareIntegers() throws {
        ...
        let gt = try requireFixture(db).query().where("value", greaterThan: .int(15)).execute()
        XCTAssertEqual(gt.count, 1)
        
        let lt = try requireFixture(db).query().where("value", lessThan: .int(15)).execute()
    ...
    func testCompareDoubles() throws {
        ...
        let gt = try requireFixture(db).query().where("value", greaterThan: .double(15.0)).execute()
        XCTAssertEqual(gt.count, 1)
    }
    ...
    func testCompareMixedNumericTypes() throws {
        ...
        let results = try requireFixture(db).query().where("value", greaterThan: .double(15.0)).execute()
        XCTAssertEqual(results.count, 1)
    }
    ...
    func testChainableAPI() throws {
        ...
        let results = try requireFixture(db)
            .query()
            .where("status", equals: .string("open"))
            .where("priority", greaterThan: .int(10))
            .orderBy("priority", descending: true)
    ...
    func testQueryBuilderWithDifferentDocumentTypes() throws {
        ...
        let highSeverity = try requireFixture(db).query()
            .where("type", equals: .string("bug"))
            .where("severity", equals: .string("high"))
            .execute()
