...
final class GarbageCollectionEdgeTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        db = try BlazeDBClient(name: "GCEdgeTest", fileURL: try requireFixture(dbURL), password: "SecureTestDB-456!")
    }
    ...
    func testGC_InsertAfterFullVacuum() async throws {
        ...
        let ids = try await requireFixture(db).insertMany((0..<20).map { i in BlazeDataRecord(["value": .int(i)]) })
        for id in ids {
            try await requireFixture(db).delete(id: id)
        }
        ...
        let emptyCount = try await requireFixture(db).count()
        XCTAssertEqual(emptyCount, 0)
        
        // Insert new records (should work normally)
        _ = try await requireFixture(db).insertMany((0..<10).map { i in BlazeDataRecord(["new": .int(i)]) })
        
        let afterInsert = try await requireFixture(db).count()
    ...
    func testGC_AsyncVacuumClearsReusablePagesBeforeFutureInserts() async throws {
        ...
        XCTAssertEqual(
            gcStatsAfter.reuseablePages,
            0,
            "VACUUM must not leave stale deleted pages pointing into compacted live data"
        )
        ...
        for (offset, id) in ids.dropFirst(2).enumerated() {
            ...
            XCTAssertEqual(
                ...
                offset + 2,
                "Insert after async VACUUM must not overwrite compacted records"
            )
        }
    ...
    func testGC_VacuumDuringActiveUse() async throws {
        ...
        let operationErrors = await errorCounter.getCount()
    ...
    func testGC_ReuseSamePageMultipleTimes() async throws {
        ...
        // Insert 1 record
        let id1 = try await requireFixture(db).insert(BlazeDataRecord(["iteration": .int(1)]))
        try await requireFixture(db).persist()
        
        let gcStats1 = try requireFixture(db).collection.getGCStats()
        ...
        for iteration in 2...10 {
            ...
            _ = try await requireFixture(db).insert(BlazeDataRecord(["iteration": .int(iteration)]))
            try await requireFixture(db).persist()
        }
        
        let gcStatsFinal = try requireFixture(db).collection.getGCStats()
    ...
    func testGC_AlternatingInsertDelete() async throws {
        ...
        for i in 0..<100 {
            if let id = currentID {
                ...
                currentID = try await requireFixture(db).insert(BlazeDataRecord(["iteration": .int(i)]))
            }
        }
        
        try await requireFixture(db).persist()
        
        let finalStats = try await requireFixture(db).getStorageStats()
        let finalCount = try await requireFixture(db).count()
    ...
    func testGC_StorageStatsAccuracy() async throws {
        ...
        var stats = try await requireFixture(db).getStorageStats()
        ...
        stats = try await requireFixture(db).getStorageStats()
    ...
    func testGC_MultipleVacuums() async throws {
        ...
        // VACUUM #1
        let vacuum1 = try await requireFixture(db).vacuum()
        print("    VACUUM 1: Reclaimed \(vacuum1.pagesReclaimed) pages")
        
        // VACUUM #2 (should be no-op)
        let vacuum2 = try await requireFixture(db).vacuum()
    ...
    func testGC_PageReuseOrder() async throws {
        ...
        // Insert 10 records
        let ids = try await requireFixture(db).insertMany((0..<10).map { i in BlazeDataRecord(["value": .int(i)]) })
        try await requireFixture(db).persist()
        ...
        _ = try await requireFixture(db).insertMany((0..<3).map { i in BlazeDataRecord(["new": .int(i)]) })
        try await requireFixture(db).persist()
    ...
    func testGC_LargeChurnScenario() async throws {
        ...
        activeIDs = try await requireFixture(db).insertMany((0..<1000).map { i in BlazeDataRecord(["value": .int(i)]) })
        try await requireFixture(db).persist()
        
        let initialSize = try await requireFixture(db).getStorageStats().fileSize
        ...
        for round in 0..<10 {
            ...
            // Add 500 new
            let newIDs = try await requireFixture(db).insertMany((0..<500).map { i in
        ...
        let finalSize = try await requireFixture(db).getStorageStats().fileSize
        let finalCount = try await requireFixture(db).count()
    ...
    func testGC_StatsConsistencyDuringOperations() async throws {
        ...
        let stats1 = try requireFixture(db).collection.getGCStats()
        ...
        let stats2 = try requireFixture(db).collection.getGCStats()
        ...
        _ = try await requireFixture(db).insertMany((0..<20).map { i in BlazeDataRecord(["new": .int(i)]) })
        try await requireFixture(db).persist()
        
        let stats3 = try requireFixture(db).collection.getGCStats()
    ...
    func testGC_AutoVacuumThresholds() async throws {
        ...
        let ids1 = try await requireFixture(db).insertMany((0..<100).map { i in BlazeDataRecord(["value": .int(i)]) })
        for i in 0..<50 {
            try await requireFixture(db).delete(id: ids1[i])  // 50% waste
        }
        try await requireFixture(db).persist()
        
        try requireFixture(db).enableAutoVacuum(wasteThreshold: 0.90, checkInterval: 0.1)
        ...
        try requireFixture(db).enableAutoVacuum(wasteThreshold: 0.40, checkInterval: 0.1)
        try await Task.sleep(nanoseconds: 500_000_000)  // 500ms
        try requireFixture(db).disableAutoVacuum()
    ...
    func testGC_PreservesRecordIDs() async throws {
        ...
        _ = try await requireFixture(db).insert(BlazeDataRecord(["id": .uuid(specificID1), "value": .int(1)]))
        _ = try await requireFixture(db).insert(BlazeDataRecord(["id": .uuid(specificID2), "value": .int(2)]))
        
        // Add more records
        _ = try await requireFixture(db).insertMany((0..<50).map { i in BlazeDataRecord(["value": .int(i)]) })
        
        // Delete many records
        let allRecords = try await requireFixture(db).fetchAll()
        ...
        let record1 = try await requireFixture(db).fetch(id: specificID1)
        let record2 = try await requireFixture(db).fetch(id: specificID2)
    ...
    func testGC_MaxReusablePages() async throws {
        ...
        // Insert 1000 records
        let ids = try await requireFixture(db).insertMany((0..<1000).map { i in BlazeDataRecord(["value": .int(i)]) })
        try await requireFixture(db).persist()
        ...
        _ = try await requireFixture(db).insertMany((0..<500).map { i in BlazeDataRecord(["new": .int(i)]) })
        try await requireFixture(db).persist()
