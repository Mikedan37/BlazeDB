...
final class MultiDatabasePatterns: XCTestCase {
    ...
    var tempDir: URL!
    private let testPassword = "MultiDB-Test-123!"
    ...
    func testPattern_CompleteECommerceSystem() async throws {
        ...
        let customers = try BlazeDBClient(name: "Customers", fileURL: tempDir.appendingPathComponent("customers.db"), password: testPassword)
        let products = try BlazeDBClient(name: "Products", fileURL: tempDir.appendingPathComponent("products.db"), password: testPassword)
        let orders = try BlazeDBClient(name: "Orders", fileURL: tempDir.appendingPathComponent("orders.db"), password: testPassword)
        let inventory = try BlazeDBClient(name: "Inventory", fileURL: tempDir.appendingPathComponent("inventory.db"), password: testPassword)
        let reviews = try BlazeDBClient(name: "Reviews", fileURL: tempDir.appendingPathComponent("reviews.db"), password: testPassword)
        ...
        // Join contract: left side provides foreign key that references right side primary key.
        let productsWithInventory = try await inventory.join(
            with: products,
            on: "product_id",
            equals: "id",
            type: .inner
        ...
        let lowStock = productsWithInventory.filter { joined in
            joined.left.storage["stock"]?.intValue ?? 999 < 10
        }
    ...
    func testPattern_MultiDatabaseAtomicity() async throws {
        ...
        let accountsDB = try BlazeDBClient(name: "Accounts", fileURL: tempDir.appendingPathComponent("accounts.db"), password: testPassword)
        let ledgerDB = try BlazeDBClient(name: "Ledger", fileURL: tempDir.appendingPathComponent("ledger.db"), password: testPassword)
    ...
    func testPattern_MasterSlaveReplication() async throws {
        ...
        let master = try BlazeDBClient(name: "Master", fileURL: tempDir.appendingPathComponent("master.db"), password: testPassword)
        let slave1 = try BlazeDBClient(name: "Slave1", fileURL: tempDir.appendingPathComponent("slave1.db"), password: testPassword)
        let slave2 = try BlazeDBClient(name: "Slave2", fileURL: tempDir.appendingPathComponent("slave2.db"), password: testPassword)
    ...
    func testPattern_MicroservicesArchitecture() async throws {
        ...
        let userService = try BlazeDBClient(name: "UserService", fileURL: tempDir.appendingPathComponent("users.db"), password: testPassword)
        let authService = try BlazeDBClient(name: "AuthService", fileURL: tempDir.appendingPathComponent("auth.db"), password: testPassword)
        let bugService = try BlazeDBClient(name: "BugService", fileURL: tempDir.appendingPathComponent("bugs.db"), password: testPassword)
        let notificationService = try BlazeDBClient(name: "NotificationService", fileURL: tempDir.appendingPathComponent("notifications.db"), password: testPassword)
    ...
    func testPattern_EventSourcing() async throws {
        ...
        let eventsDB = try BlazeDBClient(name: "Events", fileURL: tempDir.appendingPathComponent("events.db"), password: testPassword)
        let stateDB = try BlazeDBClient(name: "State", fileURL: tempDir.appendingPathComponent("state.db"), password: testPassword)
