...
final class SecurityEncryptionTests: XCTestCase {
    ...
    /// Test helper: derive token from secret + db names (BlazeDBClient does not expose deriveToken).
    private static func deriveTokenForTest(secret: String, db1: String, db2: String) -> Data {
        let input = "\(secret)|\(db1)|\(db2)".data(using: .utf8)!
    ...
    func testSecurity_EncryptionKeyRotation() async throws {
        ...
        let oldKeyDB = try? BlazeDBClient(
            name: "KeyRotation",
            fileURL: dbURL,
            password: "Old-Key-2024A!"  // OLD KEY
        )
    ...
    func testSecurity_SharedSecretRotation() async throws {
        ...
        let db1 = try BlazeDBClient(name: "SharedSecretDB1", fileURL: dbURL1, password: "Sec-Pass-1Aa!")
        let db2 = try BlazeDBClient(name: "SharedSecretDB2", fileURL: dbURL2, password: "Sec-Pass-2Aa!")

        // Simulate initial shared secret and derived token.
        // BlazeDBClient does not expose deriveToken; use test helper to validate rotation semantics.
        let oldSecret = "old-shared-secret-123"
        let newSecret = "new-shared-secret-456"

        let oldToken = Self.deriveTokenForTest(secret: oldSecret, db1: db1.name, db2: db2.name)
        let newToken = Self.deriveTokenForTest(secret: newSecret, db1: db1.name, db2: db2.name)
