...
final class MixedVersionSyncTests: XCTestCase {
    ...
    func testMixedSchema_SyncBetweenMinimalAndRichNodes() async throws {
        ...
        let v1Id = try await v1.insert(BlazeDataRecord([
            "title": .string("Legacy Bug"),
            "status": .string("open")
        ]))
        
        // Insert V3-style records on v3
        let v3Id = try await v3.insert(BlazeDataRecord([
        ...
        if let legacyOnV3 = try await v3.fetch(id: v1Id) {
            XCTAssertEqual(try legacyOnV3.string("title"), "Legacy Bug")
            XCTAssertEqual(try legacyOnV3.string("status"), "open")
        } else {
        ...
        if let modernOnV1 = try await v1.fetch(id: v3Id) {
            XCTAssertEqual(try modernOnV1.string("title"), "Modern Bug")
            XCTAssertEqual(try modernOnV1.string("status"), "open")
