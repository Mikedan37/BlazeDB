...
private actor MixedOpsCounter {
    var insertCount = 0, readCount = 0, updateCount = 0, deleteCount = 0
    func incInsert() { insertCount += 1 }
    func incRead() { readCount += 1 }
    func incUpdate() { updateCount += 1 }
    func incDelete() { deleteCount += 1 }
    func get() -> (Int, Int, Int, Int) { (insertCount, readCount, updateCount, deleteCount) }
}

private actor TxnResultCounter {
    var success = 0, fail = 0
    func record(success s: Bool) { if s { success += 1 } else { fail += 1 } }
    func get() -> (Int, Int) { (success, fail) }
}
...
final class ExtremeIntegrationTests: XCTestCase {
    ...
    func testExtreme_50DatabasesSimultaneous() async throws {
        ...
        for i in 0..<50 {
            ...
            let db = try BlazeDBClient(name: "DB\(i)", fileURL: dbURL, password: "TestPass-123456!")
            databases.append(db)
        }
    ...
    func testExtreme_ChainOf10JOINs() async throws {
        ...
        for i in 0..<10 {
            ...
            let db = try BlazeDBClient(name: "Chain\(i)", fileURL: dbURL, password: "TestPass-123456!")
            dbs.append(db)
        }
    ...
    func testExtreme_100ConcurrentTransactions() async throws {
        ...
        let db = try BlazeDBClient(name: "TxnExtreme", fileURL: dbURL, password: "TestPass-123456!")
        ...
        await withTaskGroup(of: Bool.self) { group in
            ...
            for await success in group {
                await counter.record(success: success)
            }
        }
        
        let (successCount, failCount) = await counter.get()
    ...
    func testExtreme_1000ConcurrentReads() async throws {
        ...
        let db = try BlazeDBClient(name: "ReadExtreme", fileURL: dbURL, password: "TestPass-123456!")
        ...
        let readCount = await readCounter.get()
    ...
    func testExtreme_MixedOperationsUnderLoad() async throws {
        ...
        let db = try BlazeDBClient(name: "MixedExtreme", fileURL: dbURL, password: "TestPass-123456!")
        ...
        let (insertCount, readCount, updateCount, deleteCount) = await counter.get()
        ...
        for cycle in 0..<50 {
            autoreleasepool {
                do {
                    ...
                    let db = try BlazeDBClient(name: "Rapid\(cycle)", fileURL: dbURL, password: "TestPass-123456!")
    ...
    func testExtreme_AllFeaturesSimultaneously() async throws {
        ...
        let db = try BlazeDBClient(name: "AllFeatures", fileURL: dbURL, password: "TestPass-123456!")
    ...
    func testExtreme_MillionRecordQuery() async throws {
        ...
        let db = try BlazeDBClient(name: "LargeQuery", fileURL: dbURL, password: "TestPass-123456!")
    ...
    func testExtreme_DeepRecursiveQueries() async throws {
        ...
        let db = try BlazeDBClient(name: "Recursive", fileURL: dbURL, password: "TestPass-123456!")
    ...
    func testExtreme_StressAllIndexTypes() async throws {
        ...
        let db = try BlazeDBClient(name: "IndexStress", fileURL: dbURL, password: "TestPass-123456!")
    ...
    func testExtreme_LongRunningTransaction() async throws {
        ...
        let db = try BlazeDBClient(name: "LongTxn", fileURL: dbURL, password: "TestPass-123456!")
    ...
    func testExtreme_CascadingFailures() async throws {
        ...
        let db = try BlazeDBClient(name: "Cascade", fileURL: dbURL, password: "TestPass-123456!")
        
        // Setup: 10 linked databases (simulate microservices)
        var dbs: [BlazeDBClient] = []
        for i in 0..<10 {
            let url = tempDir.appendingPathComponent("service\(i).blazedb")
            let serviceDB = try BlazeDBClient(name: "Service\(i)", fileURL: url, password: "TestPass-123456!")
