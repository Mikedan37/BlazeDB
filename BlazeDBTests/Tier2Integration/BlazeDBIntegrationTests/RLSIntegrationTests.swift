...
final class RLSIntegrationTests: XCTestCase {
    ...
    func testRLS_BugTrackerScenario() throws {
        ...
        let db = try BlazeDBClient(name: "Bugs", fileURL: dbURL, password: "TestPass-123456!")
        ...
        // Use persisted user state from AccessManager so team memberships are included.
        func contextFor(_ userID: UUID) throws -> SecurityContext {
            guard let user = db.rls.getUser(id: userID) else {
                throw NSError(
                    domain: "RLSIntegrationTests",
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "User not found for context: \(userID)"]
                )
            }
        ...
        db.rls.setContext(try contextFor(aliceID))
        ...
        db.rls.setContext(try contextFor(bobID))
        ...
        db.rls.setContext(try contextFor(adminID))
    ...
    func testRLS_UserOwnsRecordScenario() throws {
        ...
        let db = try BlazeDBClient(name: "Tasks", fileURL: dbURL, password: "TestPass-123456!")
    ...
    func testRLS_ViewerReadOnlyScenario() throws {
        ...
        let db = try BlazeDBClient(name: "Docs", fileURL: dbURL, password: "TestPass-123456!")
    ...
    func testRLS_MultiTenantScenario() throws {
        ...
        let db = try BlazeDBClient(name: "SaaS", fileURL: dbURL, password: "TestPass-123456!")
    ...
    func testRLS_HierarchicalPermissions() throws {
        ...
        let db = try BlazeDBClient(name: "Hierarchy", fileURL: dbURL, password: "TestPass-123456!")
