...
final class SoakStressTests: XCTestCase {
    ...
    func testSoak_Sync_Vacuum_GC_Coexist() async throws {
        ...
        for i in 0..<iterations {
            // Write on db1
            let id1Local = try await db1.insert(BlazeDataRecord([
                "source": .string("db1"),
                "seq": .int(i)
            ]))
            ...
            // Write on db2
            let id2Local = try await db2.insert(BlazeDataRecord([
                "source": .string("db2"),
                "seq": .int(i)
            ]))
            ...
            if i % 10 == 0, let any = idsDB1.randomElement(),
               var rec = try await db1.fetch(id: any) {
                rec.storage["updated"] = .bool(true)
                try await db1.update(id: any, with: rec)
            }

            if i % 10 == 0, let any = idsDB2.randomElement(),
               var rec = try await db2.fetch(id: any) {
                rec.storage["updated"] = .bool(true)
                try await db2.update(id: any, with: rec)
            }

            // Periodic sync delay
            if i % 200 == 0 {
                try await Task.sleep(nanoseconds: 200_000_000) // 200ms
                try await db1.persist()
                try await db2.persist()
        ...
        let all1 = try await db1.fetchAll()
        let all2 = try await db2.fetchAll()
