...
final class RLSNegativeTests: XCTestCase {
    ...
    func testSync_RespectsRLSWhenEnabled() async throws {
        ...
        let secretID = try await server.insert(BlazeDataRecord([
        ...
        let serverNode = try await topology.register(db: server, name: "ServerRLS", role: .server)
        let clientNode = try await topology.register(db: client, name: "ClientRLS", role: .client)
        ...
        let outsiderVisible = try client.rls.filterRecords(operation: .select, records: try await client.fetchAll())
        ...
        let ownerVisible = try client.rls.filterRecords(operation: .select, records: try await client.fetchAll())
        XCTAssertEqual(ownerVisible.count, 1)
        XCTAssertEqual(try ownerVisible.first?.string("title"), "Owner Secret")
        
        // And the record should still exist on server
        let serverRecord = try await server.fetch(id: secretID)
        XCTAssertNotNil(serverRecord)
    }
}
