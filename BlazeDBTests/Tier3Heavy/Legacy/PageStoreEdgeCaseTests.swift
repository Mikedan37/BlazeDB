...
final class PageStoreBoundaryTests: XCTestCase {
    ...
    /// AES-GCM adds 12-byte nonce + 16-byte auth tag = 28 bytes on top of the 9-byte header.
    private let aesGcmOverhead = 12 + 16  // nonce + auth tag

    /// Write/read exactly the max plaintext that fits in one encrypted page.
    /// Page layout: header(4) + version(1) + length(4) + nonce(12) + tag(16) + ciphertext.
    func testMaxPayloadRoundTrip() throws {
        let url = tmpURL()
        let key = SymmetricKey(size: .bits256)
        let pageSize = try inferPageSize(at: url, key: key)

        let maxPayload = pageSize - 9 - aesGcmOverhead
    ...
    /// Attempt to write one byte more than max — should fail because it would overflow the page.
    func testTooLargePayloadThrows() throws {
        let url = tmpURL()
        let key = SymmetricKey(size: .bits256)
        let pageSize = try inferPageSize(at: url, key: key)

        let tooLarge = pageSize - 9 - aesGcmOverhead + 1
