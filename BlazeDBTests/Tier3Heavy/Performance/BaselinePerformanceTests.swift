...
final class BaselinePerformanceTests: XCTestCase {
    ...
    /// Perf baselines are noisy across machines/CI loads.
    /// Opt into hard regression failures with BLAZEDB_ENFORCE_PERF_BASELINE=1.
    ...
    override func setUp() {
        ...
        db = try! BlazeDBClient(name: "baseline_test", fileURL: tempURL, password: "SecureTestDB-456!")
    }
    ...
    private func recordBenchmark(name: String, time: Double, allowedRegression: Double = 0.20) {
        ...
        if let existing = baselines[name] {
            ...
            // Older/invalid baseline snapshots can contain zero timings (e.g. failed runs).
            // Treat those as missing and rebuild baseline from the current valid measurement.
            if baseline <= 0.000_1 {
                print("\n📊 BASELINE RESET (invalid prior value): \(name)")
                print("   Previous baseline: \(String(format: "%.6f", baseline))s")
                print("   New baseline:      \(String(format: "%.3f", time))s")
                baselines[name] = Baseline(
                    testName: name,
                    averageTime: time,
                    lastRun: Date(),
                    runCount: max(existing.runCount, 1),
                    stdDeviation: 0.0
                )
            ...
            if regression > allowedRegression && enforceRegressionGate {
                XCTFail("""
                    ...
                    """)
            } else if regression > allowedRegression {
                print("   ⚠️ Regression exceeds threshold, but gate is disabled (set BLAZEDB_ENFORCE_PERF_BASELINE=1 to fail)")
