...
/// Swift 6 Sendable-safe container for concurrent mutation from closures
private final class LockedArray<T>: @unchecked Sendable {
    private let lock = NSLock()
    private var storage: [T] = []
    func append(_ element: T) { lock.lock(); defer { lock.unlock() }; storage.append(element) }
...
final class PropertyBasedTests: XCTestCase {
    ...
    override func setUp() {
        ...
        db = try! BlazeDBClient(name: "prop_test", fileURL: tempURL, password: "SecureTestDB-456!")
    }
    ...
    private static func randomField() -> BlazeDocumentField {
        ...
        case 7:
            ...
            let array = (0..<count).map { _ in Self.randomSimpleField() }
            return .array(array)
        ...
        case 8:
            ...
            for i in 0..<count {
                dict["key\(i)"] = Self.randomSimpleField()
            }
    ...
    private static func randomSimpleField() -> BlazeDocumentField {
    ...
    func testProperty_InsertFetchRoundTrip() throws {
        ...
        for id in insertedIDs {
            ...
            // Compare each originally inserted field (ignore engine-managed fields like id/timestamps)
    ...
    func testProperty_UpdatePreservesOtherFields() throws {
        ...
        for i in 0..<200 {
            ...
            // Update only field1 via partial-update API.
            ...
            // Never hard-crash this fuzz/property test on optional unwrapping.
            guard
                let updatedField2 = updated["field2"], let originalField2,
                let updatedField3 = updated["field3"], let originalField3,
                let updatedField4 = updated["field4"], let originalField4,
                let updatedField5 = updated["field5"], let originalField5
            else {
                failures += 1
                continue
            }
            if !valuesEqual(updatedField2, originalField2) { failures += 1 }
            if !valuesEqual(updatedField3, originalField3) { failures += 1 }
            if !valuesEqual(updatedField4, originalField4) { failures += 1 }
            if !valuesEqual(updatedField5, originalField5) { failures += 1 }
    ...
    func testProperty_InsertOrderIndependence() throws {
        ...
        BlazeDBClient.clearCachedKey()
        let db1 = try BlazeDBClient(name: "prop1", fileURL: db1URL, password: "SecureTestDB-456!")
        ...
        BlazeDBClient.clearCachedKey()
        let db2 = try BlazeDBClient(name: "prop2", fileURL: db2URL, password: "SecureTestDB-456!")
        ...
        XCTAssertEqual(try db1.count(), try db2.count(), "Count should be same regardless of insert order")
    ...
    func testProperty_TransactionAtomicity() throws {
        ...
        for i in 0..<100 {
            ...
            let records = (0..<recordCount).map { _ in Self.randomRecord() }
            
            do {
                _ = try db.insertMany(records)
                
                // Property: All should be inserted
                let newCount = try db.count()
    ...
    func testProperty_ConcurrentSafety() throws {
        ...
        let errorsBox = LockedArray<Error>()
        ...
        for i in 0..<1000 {
            ...
            DispatchQueue.global().async {
                ...
                do {
                    ...
                    switch op {
                    case 0:  // Insert
                        _ = try db.insert(Self.randomRecord())
                    case 1:  // Fetch
                        _ = try db.fetchAll()
                    case 2:  // Update
                        let all = try db.fetchAll()
        ...
        print("  📊 Errors: \(errorsBox.values.count)")
    ...
    private func valuesEqual(_ a: BlazeDocumentField, _ b: BlazeDocumentField) -> Bool {
        ...
        case (.array(let a), .array(let b)):
            ...
            return zip(a, b).allSatisfy(valuesEqual)
        case (.dictionary(let a), .dictionary(let b)):
            guard a.count == b.count else { return false }
            for (key, valueA) in a {
                guard let valueB = b[key], valuesEqual(valueA, valueB) else { return false }
            }
            return true
        case (.vector(let a), .vector(let b)):
            guard a.count == b.count else { return false }
            return zip(a, b).allSatisfy { abs($0 - $1) < 0.0001 }
        case (.null, .null):
            return true
