...
final class DistributedGCPerformanceTests: XCTestCase {
    ...
    func testPerformance_SyncStateGC_Compact() async throws {
        ...
        for i in 0..<10000 {
            _ = try requireFixture(db1).insert(BlazeDataRecord(["value": .int(i)]))
        }
    ...
    func testPerformance_MultiDatabaseGC_Coordinate() async throws {
        ...
        for i in 0..<10 {
            ...
            let db = try BlazeDBClient(name: "DB\(i)", fileURL: url, password: "SecureTestDB-456!")
            databases.append(db)
...
#else
final class DistributedGCPerformanceTests: XCTestCase {
    func testDistributedGCPerformanceUnavailableInCoreBuild() {
        XCTAssertTrue(true, "Distributed GC performance tests require distributed build")
    }
}
#endif
