...
final class AggregationTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        for _ in 0..<3 {
            ...
            try? FileManager.default.removeItem(at: url.deletingLastPathComponent().appendingPathComponent("transaction_backup.blazedb"))
            try? FileManager.default.removeItem(at: url.deletingLastPathComponent().appendingPathComponent("transaction_backup.meta"))
            try? FileManager.default.removeItem(at: url.deletingLastPathComponent().appendingPathComponent("txn_log.json"))
        ...
        db = try BlazeDBClient(name: "agg_test_\(testID)", fileURL: url, password: "SecureTestDB-456!")
    ...
    func testSumWithDouble() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["price": .double(10.5)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["price": .double(20.25)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["price": .double(5.75)]))
    ...
    func testMin() throws {
        ...
        _ = try requireFixture(db).insert(BlazeDataRecord(["value": .int(5)]))
    ...
    func testGroupByWithSum() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["team": .string("A"), "hours": .double(10.5)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["team": .string("A"), "hours": .double(20.0)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["team": .string("B"), "hours": .double(15.0)]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["team": .string("B"), "hours": .double(25.5)]))
    ...
    func testGroupByOnMissingField() throws {
        _ = try requireFixture(db).insert(BlazeDataRecord(["field1": .string("value")]))
        _ = try requireFixture(db).insert(BlazeDataRecord(["field2": .string("value")]))
    ...
    func testAggregationWithNilValues() throws {
        ...
        print("  Insert 2...")
        _ = try requireFixture(db).insert(BlazeDataRecord(["other": .string("no value field")]))
        ...
        let query = try requireFixture(db).query().sum("value", as: "sum")
        print("  ✅ Query built")
    ...
    func testAggregationPerformanceOn10k() throws {
        ...
        XCTAssertEqual(result.groups.count, 3)
        XCTAssertLessThan(duration, 4.0, "Aggregation on 10k records should be < 4s")
    }
    ...
    func testBugTrackerDashboard() throws {
        ...
        let statusStats = try requireFixture(db).query()
            .where("type", equals: .string("bug"))
            .groupBy("status")
            .aggregate([
                .count(as: "count"),
        ...
        // Severity breakdown
        let severityStats = try requireFixture(db).query()
            .where("type", equals: .string("bug"))
            .where("status", equals: .string("open"))
            .groupBy("severity")
            .count()
