...
final class BlazeDBBackupTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        dbURL = url
        db = try BlazeDBClient(name: "BackupTest", fileURL: url, password: "SecureTestDB-456!")
    }
    ...
    func testBackup_CreatesValidBackup() async throws {
        ...
        // Create backup
        let backupURL = try requireFixture(dbURL).deletingLastPathComponent()
        ...
        let stats = try await requireFixture(db).backup(to: backupURL)
    ...
    func testRestore_RestoresFromBackup() async throws {
        ...
        _ = try await requireFixture(db).insertMany(original)
        ...
        _ = try await requireFixture(db).backup(to: backupURL)
        
        // Modify database (add more records)
        _ = try await requireFixture(db).insertMany((0..<30).map { i in BlazeDataRecord(["new": .int(i)]) })
        
        let beforeRestore = try await requireFixture(db).count()
        XCTAssertEqual(beforeRestore, 50, "Should have 20 + 30 = 50")
        
        // Restore from backup
        try await requireFixture(db).restore(from: backupURL)
        
        let afterRestore = try await requireFixture(db).count()
    ...
    func testConcurrentBackup() async throws {
        ...
        let parent = try requireFixture(dbURL).deletingLastPathComponent()
        let backupURLs = (0..<5).map { i in
            parent.appendingPathComponent("concurrent-\(i)-\(UUID().uuidString).blazedb")
        }
        ...
        let client = try XCTUnwrap(self.db)
        await withTaskGroup(of: Bool.self) { group in
            for url in backupURLs {
                group.addTask {
                    do {
                        _ = try await client.backup(to: url)
                        return true
                    } catch {
    ...
    func testExportJSON() async throws {
        ...
        _ = try await requireFixture(db).insertMany((0..<10).map { i in
            BlazeDataRecord(["value": .int(i)])
        })
        
        let jsonData = try await requireFixture(db).export(format: .json)
    ...
    func testExportCBOR() async throws {
        ...
        let cborData = try await requireFixture(db).export(format: .cbor)
    ...
    func testExportFiltered() async throws {
        ...
        _ = try await requireFixture(db).insertMany((0..<50).map { i in
            BlazeDataRecord([
                "value": .int(i),
        ...
        let filteredData = try await requireFixture(db).export(
    ...
    func testImportAppendMode() async throws {
        ...
        let stats = try await requireFixture(db).import(from: exportData, format: .json, mode: .append)
    ...
    func testImportReplaceMode() async throws {
        ...
        let beforeImport = try await requireFixture(db).count()
        ...
        let stats = try await requireFixture(db).import(from: newData, format: .json, mode: .replace)
        
        XCTAssertEqual(stats.recordsImported, 5)
        
        let afterImport = try await requireFixture(db).count()
    ...
    func testImportMergeMode() async throws {
        ...
        let id1 = UUID()
        let id2 = UUID()
        _ = try await requireFixture(db).insert(BlazeDataRecord(["id": .uuid(id1), "value": .int(1), "status": .string("old")]))
        _ = try await requireFixture(db).insert(BlazeDataRecord(["id": .uuid(id2), "value": .int(2), "status": .string("old")]))
        ...
        let stats = try await requireFixture(db).import(from: importData, format: .json, mode: .merge)
        ...
        let updated = try await requireFixture(db).fetch(id: id1)
        ...
        let unchanged = try await requireFixture(db).fetch(id: id2)
    ...
    func testExportImportRoundTrip() async throws {
        ...
        _ = try await requireFixture(db).insertMany(originalRecords)
        
        // Export
        let jsonData = try await requireFixture(db).export(format: .json)
        
        // Create new database
        let newDBURL = try requireFixture(dbURL).deletingLastPathComponent()
            .appendingPathComponent("roundtrip-\(UUID().uuidString).blazedb")
        let newDB = try BlazeDBClient(name: "RoundTrip", fileURL: newDBURL, password: "SecureTestDB-456!")
    ...
    func testIncrementalBackup() async throws {
        ...
        // Insert new records
        _ = try await requireFixture(db).insertMany((0..<5).map { i in
            BlazeDataRecord([
                "value": .int(i + 100),
        ...
        let stats = try await requireFixture(db).incrementalBackup(to: backupURL, since: twelveHoursAgo)
    ...
    func testVerifyBackup() async throws {
        ...
        // Verify valid backup
        let isValid = try await requireFixture(db).verifyBackup(at: backupURL)
    ...
    func testImportWithDuplicateIDs() async throws {
        ...
        _ = try await requireFixture(db).insert(BlazeDataRecord(["id": .uuid(sharedID), "value": .int(1)]))
        ...
        let stats = try await requireFixture(db).import(from: duplicateData, format: .json, mode: .append)
        
        XCTAssertEqual(stats.recordsSkipped, 1, "Duplicate should be skipped")
        XCTAssertEqual(stats.recordsImported, 0)
        
        // Verify original value unchanged
        let record = try await requireFixture(db).fetch(id: sharedID)
    ...
    func testImportInvalidJSON() async throws {
        ...
        do {
            _ = try await requireFixture(db).import(from: invalidData, format: .json, mode: .append)
    ...
    func testPerformance_Backup() async throws {
        ...
        let dbURL = try requireFixture(self.dbURL)
        measure(metrics: [XCTClockMetric(), XCTMemoryMetric(), XCTStorageMetric()]) {
            let done = expectation(description: "backup performance iteration")
            Task {
                do {
    ...
    func testPerformance_ExportImport() async throws {
        ...
        measure(metrics: [XCTClockMetric(), XCTMemoryMetric()]) {
            let done = expectation(description: "export/import performance iteration")
            Task {
                do {
                    _ = try await db.insertMany((0..<100).map { i in BlazeDataRecord(["value": .int(i)]) })
                    
                    let data = try await db.export(format: .json)
                    _ = try await db.import(from: data, format: .json, mode: .append)
                } catch {
