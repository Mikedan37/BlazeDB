...
final class EncryptionRoundTripTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        db = try BlazeDBClient(name: "EncryptionTest", fileURL: try requireFixture(dbURL), password: "SecureTestDB-456!")
    }
    ...
    func testEncryption_BasicRoundTrip() throws {
        ...
        let id = try requireFixture(db).insert(originalData)
        
        // Fetch data (should be decrypted)
        let retrieved = try requireFixture(db).fetch(id: id)
    ...
    func testEncryption_AllDataTypes() throws {
        ...
        let id = try requireFixture(db).insert(testData)
        let retrieved = try requireFixture(db).fetch(id: id)
    ...
    func testEncryption_WrongPassword() throws {
        ...
        let db1 = try BlazeDBClient(name: "DB1", fileURL: dbURL1, password: "SecurePass-12345!")
    ...
    func testEncryption_DetectsCorruption() throws {
        ...
        let fileHandle = try FileHandle(forUpdating: try requireFixture(dbURL))
        ...
        do {
            let db2 = try BlazeDBClient(name: "EncryptionTest", fileURL: try requireFixture(dbURL), password: "SecureTestDB-456!")
            _ = try requireFixture(db2).fetch(id: id)
    ...
    func testEncryption_UniqueNoncesPerPage() throws {
        ...
        let fileData = try Data(contentsOf: try requireFixture(dbURL))
