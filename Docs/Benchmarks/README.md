...
./bench smoke            # honesty + short sweeps (debug)
./bench payload --release
./bench db-size --release
./bench full             # honesty + release db-size + all payload paths (long)
./bench publish          # full + write Docs/Benchmarks/payload_size_sweep.md
...
- For **payload size vs latency**, use [PAYLOAD_SIZE.md](PAYLOAD_SIZE.md) (`BLAZEDB_BENCH_MODE=payload_size_sweep`) — do not infer core-row size from the 1 MB growth limit.
- For **write latency vs DB size**, use `./Scripts/run_db_size_sweep.sh` (`BLAZEDB_BENCH_MODE=db_size_sweep`).
