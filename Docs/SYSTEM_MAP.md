...
| `TypedStore<T>` | Stable | Public | `Codable/TypedStore.swift` | — | `db.typed(T.self)` — CRUD + KeyPath queries; `fetchAll()`, `TypeSafeQueryBuilder.all()`, and `count()` filter by decodability, silently skipping records that cannot decode as `T` |
