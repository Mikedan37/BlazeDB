...
#### **Scenario:**
```
• 1,000 users in a group chat
• Each user sends 10 messages per minute
• Each message is ~100 bytes
• Total: 10,000 messages per minute = 167 messages per second
```
...
• Each post is ~500 bytes
• Total: 1,000,000 posts per day = 11.6 posts per second
```
...
### **3. IoT Sensor Network (Like Smart Home):**

#### **Scenario:**
```
• 100 smart home devices
• Each device sends 1 reading per second
• Each reading is ~50 bytes
• Total: 100 readings per second
```
...
• Each user makes 10 edits per minute
• Each edit is ~200 bytes
• Total: 500 edits per minute = 8.3 edits per second
```
...
### **5. Multiplayer Game (Like Fortnite):**

#### **Scenario:**
```
• 100 players in a game
• Each player sends 60 updates per second (60 FPS)
• Each update is ~150 bytes
• Total: 6,000 updates per second
```
...
### **6. File Sync (Like Dropbox/iCloud):**

#### **Scenario:**
```
• User uploads 1GB file
• File is split into 1MB chunks
• Total: 1,000 chunks
• Each chunk is ~1MB = 1,048,576 bytes
```
...
#### **Realistic Network Limit:**
```
• WiFi (100 Mbps): 12.5 MB/s
• 1GB file: 1,000 MB
• Time to sync: 1,000 / 12.5 = 80 seconds (network limited)

Result: Network is the bottleneck, not BlazeDB!
```
...
• WiFi (100 Mbps): 12.5 MB/s
• 10GB file: 10,000 MB
• Time to backup: 10,000 / 12.5 = 800 seconds = 13.3 minutes
...
### **4. Enterprise App (10,000,000 users):**
```
...
• 60,000,000 operations per minute
• 3,600,000,000 operations per hour
...
#### **Battery Life:**
```
• Light usage: 0.1W → 32,000 hours = 1,333 days = 3.65 YEARS
...
```
Devices: 100
Readings: 1 per device per second
