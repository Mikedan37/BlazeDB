...
**Date:** 2026-07-26  
**Repo state:** `main` @ `3ae19461` (after stash recovery cherry-picks)  
...
- **July 2026 is the strongest recent discipline window:** narrower subjects, **0** commits >30 files that month, honesty/audit/tooling commits that preserve reviewable intent, and the stash-recovery series split by concern instead of one “restore WIP” blob.
...
**20% mixed is high for textbook “one concern per commit,”** but many mixes are intentional product ships. The failure mode is not “docs with a test,” it is **unrelated residue riding a CI fix**.

Recent recovery commits consciously avoided that: storage / MVCC docs / test fix / benches / examples / Homebrew / Xcode scheme each got their own commit.
...
- Adjacent fixes are often CI flake follow-ups, not “commit then instantly undo.” Still, chains of `fix(ci)` are a social smell; prefer one commit with evidence or a PR series with named flakes.
...
**Yes for recent work.** July subjects name the *why* (“Batch WAL fsyncs…”, “Ground the roadmap in a product audit…”, “Keep KeyManager.legacyPasswordSalt…” was avoided by merge, salt preserved).  
...
| Period | Character |
|--------|-----------|
| **Strongest (recent)** | **2026-07** — honesty docs, product boundaries, `./dev`, narrow commits, stash recovered as split cherry-picks |
...
| Dimension | Trend |
|-----------|--------|
| Scope | July: **0** oversized commits; median still small |
...
Remaining gap: **history residue** (old mega commits, branch zoo, Cursor attribution trailers in older bodies) still exists. Do not rewrite published history; explain it and keep landing narrow commits.
...
- First `./dev tier0` attempt in the recovery session failed on **arch mismatch** (arm64 test bundle vs x86_64 loader); clean `arch -arm64` run executed **199** selected tests with **0 failures**, plus `swift run HelloBlazeDB` success — environment caveat, not a product regression signal.
