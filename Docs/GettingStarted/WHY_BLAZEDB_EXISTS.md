...
The result is a local-first database engine that provides ACID guarantees, concurrent access without blocking, offline-first synchronization, end-to-end encryption, all in a single, Swift-native package. (Row-level security is under development and not yet available.) Developers get correctness, safety, and real distributed behavior without compromising on performance or platform integration.
